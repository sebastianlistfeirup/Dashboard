...
function buildEngagement(rows, minBucket) {
  ...
  return {
    ...
    distribution: [
      { label: 'Har aldrig åbnet', n: withMail.filter((r) => r.opened === 0).length },
      { label: 'Åbner sjældent (under 20 %)', n: withMail.filter((r) => openShare(r) > 0 && openShare(r) < 0.2).length },
      { label: 'Åbner nogle gange (20–50 %)', n: withMail.filter((r) => openShare(r) >= 0.2 && openShare(r) < 0.5).length },
